...
const stationsUrl = 'http://165.22.83.21:3000/api/employees/stations';
...
export class StationsService {
  ...
  getPermissions(): Observable<any> {
    return this.http.get('http://165.22.83.21:3000/api/Stations/permissions');
  }
  ...
  changeStation(taskId: string, oldNumber: string): Observable<any> {
    const dialogData = {
      taskId,
      oldNumber,
      stations: this.getStations().pipe(
        map(stations => stations.map(station => station.stationNumber))
      )
    };
    ...
    return dialogRef.afterClosed().pipe(
      filter(val => !!val),
      switchMap(stationNumber =>
        this.http.put(`${taskUrl}/change/${taskId}`, { stationNumber })
      )
    );
  }
}
