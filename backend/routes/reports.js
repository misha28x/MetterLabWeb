...
router.get("/rejected/single/:date/:createFor", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `taskDate` = '" + req.params.date +
    "' AND `status` LIKE '%ідхилено%' AND `createFor` = '" + req.params.createFor + "';", (err, result) => {
      if (err) {
        console.log(err);
      }
...
// Звіт: конверт 
// створено для
router.get("/convert/single/:date/:createFor", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `addingDate` = '" + req.params.date + "' AND `createFor` = '" + req.params.createFor + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Конверт за " + req.params.date;
    generateReportExcel(result, stringName).then(name => {
      res.download(name);
    });
  });
...
/**
 * Звіти для операторів
 * 
 * @param userId - параметр для вибірок
 */

// Звіт: `заявки в роботі` (по даті додання зі статусом `Визначено відповідальну особу`)
// створено для
router.get("/operator/in-progress/single/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `addingDate` = '" + req.params.date + "' AND (`status` = 'Визначено відповідальну особу' OR `status` = 'В роботі') AND `userId` = '" + req.params.userId + "';", (err, result) => {
    const stringName = "Заявки в роботі " + req.params.date;
    generateReportExcel(result, stringName).then(name => {
      res.download(name);
    });
...
// Звіт: `заявки в роботі` для діапазону (по даті додання зі статусом `Визначено відповідальну особу`)
// створено для
router.get("/operator/in-progress/range/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE (`addingDate` BETWEEN '" + req.params.date.split(',')[0] + "' AND '" + req.params.date.split(',')[1] + "') AND (`status` = 'Визначено відповідальну особу' OR `status` = 'В роботі') AND `userId` = '" + req.params.userId + "';", (err, result) => {
    console.log({
      dates: req.params.date
    });
...
// Звіт: `по виконаних завданнях` (по `task_date` зі статусом like `Проведено повірку%`)
// створено для
router.get("/operator/completed/single/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `taskDate` = '" + req.params.date + "' AND `status` LIKE 'Проведено повірку%' AND `userId` = '" + req.params.userId + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Виконані заявки " + req.params.date;
...
// Звіт: `по виконаних завданнях` для діапазону дат (по `task_date` зі статусом like `Проведено повірку%`)
// створено для
router.get("/operator/completed/range/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE (`taskDate` BETWEEN '" + req.params.date.split(',')[0] + "' AND '" + req.params.date.split(',')[1] + "') AND `status` LIKE 'Проведено повірку%' AND `userId` = '" + req.params.userId + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Виконані заявки з " + req.params.date.split(',')[0] + "-" + req.params.date.split(',')[1];
...
// Звіт: `по відхиленим повіркам` (по `task_date` зі статусом like `%ідхилено%`)
// створено для
router.get("/operator/rejected/single/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `taskDate` = '" + req.params.date +
    "' AND `status` LIKE '%ідхилено%' AND `userId` = '" + req.params.userId + "';", (err, result) => {
      if (err) {
        console.log(err);
      }
      const stringName = "Відхилені повірки " + req.params.date;
...
// Звіт: `по відхиленим повіркам` для діапазону дат (по `task_date` зі статусом like `%ідхилено%`)
// створено для
router.get("/operator/rejected/range/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE (`taskDate` BETWEEN '" + req.params.date.split(',')[0] + "' AND '" + req.params.date.split(',')[1] + "') AND `status` LIKE '%ідхилено%' AND `userId` = '" + req.params.userId + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Відхилені повірки " + req.params.date.split(',')[0] + "-" + req.params.date.split(',')[1];
...
// Звіт: конверт 
// створено для
router.get("/operator/convert/single/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `addingDate` = '" + req.params.date + "' AND `userId` = '" + req.params.userId + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Конверт за " + req.params.date;
...
// Звіт: конверт (все в діапазоні `Дата додання`)
// створено для
router.get("/operator/convert/range/:date/:userId", (req, res, next) => {
  connection.query("SELECT * FROM `archive` WHERE `addingDate` BETWEEN '" + req.params.date.split(',')[0] + "' AND '" + req.params.date.split(',')[1] + "' AND `userId` = '" + req.params.userId + "';", (err, result) => {
    if (err) {
      console.log(err);
    }
    const stringName = "Конверт з " + req.params.date.split(',')[0] + "-" + req.params.date.split(',')[1];
